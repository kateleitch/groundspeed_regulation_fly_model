...
wind_direction_list = np.linspace(0,2*np.pi,4, endpoint = False)#in radians, the direction TO WHICH the wind is blowing. NOTE THAT THIS IS PI RADIANS OFF MY TYPICAL CONVENTION
wind_speed_list = np.linspace(0,4.2, 6) #in meters per second
...
def calculate_trajectory_vector (wind_speed, wind_dir,
        ...
        a = np.linalg.norm(attempted_airspeed_vector_along_body_axis)
        if reverse_airspeed_limit < a < forward_airspeed_limit:
            ...
            actual_airspeed_vector_along_body_axis = forward_airspeed_limit
...
for wind_dir in wind_direction_list:
    for wind_speed in wind_speed_list:
        experiment_dictionary = {'trajectories':[],'groundspeeds along trajectories':[], 'windspeeds along trajectories':[]}
        # trap_dictionary = {}
        # for i in range (trap_number):
        #     trap_dictionary[str(trap_angle_list[i])] = {'groundspeed_along_trajectory':[], 'windspeed_along_trajectory':[]}


        for fly_heading in fly_headings:
            wind_vector, trajectory_vector = calculate_trajectory_vector(wind_speed, wind_dir,
                                        fly_heading, preferred_groundspeed_along_body_axis,
                                        forward_airspeed_limit, reverse_airspeed_limit,
                                        weight_of_perpendicular_slip, ax_handle)
        ...
        dictionary[str(wind_speed)+'_m/s to_'+str(wind_dir)]=experiment_dictionary

            # #now asking if trajectory vector will bring fly to a trap
            # for angle_to_trap in trap_angle_list:
            #     if (angle_to_trap -trap_angular_width/2.) <= trajectory_angle <= (angle_to_trap + trap_angular_width/2.):
            #         windspeed_along_trajectory = scalar_projection(wind_vector, trajectory_vector)
            #         groundspeed_along_trajectory = np.linalg.norm(trajectory_vector)
            #         ax_handle.scatter(windspeed_along_trajectory, groundspeed_along_trajectory , color = [0,0,0,0.1])
            #         trap_dictionary[str(angle_to_trap)]['groundspeed_along_trajectory'].append(groundspeed_along_trajectory)
            #         trap_dictionary[str(angle_to_trap)]['windspeed_along_trajectory'].append(windspeed_along_trajectory)
        ...
        #         ax_handle.errorbar(mean_windspeed_along_trajectory, mean_groundspeed_along_trajectory, xerr=err_windspeed_along_trajectory, yerr = err_groundspeed_along_trajectory, fmt='o', color = 'green') #, label = key)

with open('./simulated_groundspeeds_vs_windspeeds_along_trajectories.json', mode = 'w') as f:
    json.dump(dictionary,f, indent = 1)

# ax_handle.set_ylabel('groundspeed along trajectory, m/s')
# ax_handle.set_xlabel('windspeed along trajectory, m/s')
# ax_handle.set_ylim(0, 4)
# ax_handle.axis('equal')
# plt.axhline(preferred_groundspeed_along_body_axis, color = 'k', linestyle = '--')
# domain = np.linspace(0,4,500)
# #[x+1 if x >= 45 else x+5 for x in l]
# ax_handle.plot(domain, domain, '--k')
# plt.show()
